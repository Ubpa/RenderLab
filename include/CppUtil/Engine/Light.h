...
namespace CppUtil {
	namespace Engine {
		...
		public:
			...
			virtual RGBf Sample_L(const Point3 & p, Normalf & wi, float & distToLight, float & PD) const = 0;

			// 获取 L
			// 如果不能击中光源，则返回 0
			virtual RGBf GetL(const Point3 & p, const Vec3 & dirToLight, float & distToLight) = 0;

			// 获取 L
			// 如果 p 不可能被照射到，则返回 vec3(0)
			virtual RGBf GetMaxL(const Point3 & p) const = 0;
