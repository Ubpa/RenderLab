...
namespace CppUtil {
	namespace Engine {
		...
		public:
			Ray(const Pointf & origin = Pointf(0), const Pointf & dir = Pointf(1), float tMin = 0.001f, float tMax = FLT_MAX)
				: Basic::Ray(origin, dir), tMin(tMin), tMax(tMax) { }

		public:
			void Init(const Pointf & origin, const Vectorf & dir) {
				o = origin;
				d = dir;
				tMin = 0.001f;
				tMax = FLT_MAX;
			...
			const Pointf StartPos() const { return (*this)(tMin); }
			const Pointf EndPos() const { return (*this)(tMax); }
