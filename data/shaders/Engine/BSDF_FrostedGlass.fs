...
vec3 BSDF(vec3 norm, vec3 wo, vec3 wi, vec3 color, float roughness, float ao, float ior) {
	...
	if (isReflect) {
		...
		h *= sign(dot(h, norm));// 使 h 指向外侧
		return color * Fr(wo, h, ior) * GGX_D(norm, h, alpha) * GGX_G(norm, wo, wi, h, alpha) / abs(4.f * NoWo * NoWi);
	}
	else {
		...
		h *= sign(dot(h,norm));// 使 h 指向外侧
		return color * factor * ((1 - Fr(wo, h, ior)) * GGX_D(norm, h, alpha) * GGX_G(norm, wo, wi, h, alpha) * etat * etat) /
			(sqrtDenom * sqrtDenom);
	}
}
