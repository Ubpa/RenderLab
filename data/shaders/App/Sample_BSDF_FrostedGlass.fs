...
layout (location = 0) out vec3 gDirectIllum;
layout (location = 1) out vec3 gPosition;
layout (location = 2) out vec3 gViewDir;
layout (location = 3) out vec3 gNormal;
layout (location = 4) out vec3 gMat_Color;
layout (location = 5) out vec3 gMat_IOR_Roughness_ID;
...
void main() {
	...
	gNormal = norm;
	gMat_Color = ao * color;
	gMat_IOR_Roughness_ID = vec3(bsdf.ior, roughness, ID);
}
