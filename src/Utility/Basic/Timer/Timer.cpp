...
bool Timer::Start() {
	switch (state)
	{
	case Other::Timer::ENUM_STATE_INIT:
		wholeTime = 0;
		logs.push_front(0);
		// no break
	case Other::Timer::ENUM_STATE_STOP:
...
bool Timer::Stop() {
	switch (state)
	{
	case Other::Timer::ENUM_STATE_RUNNING: {
...
double Timer::GetWholeTime() const {
	switch (state)
	{
	case CppUtility::Other::Timer::ENUM_STATE_INIT:
		return 0;
		break;
	case CppUtility::Other::Timer::ENUM_STATE_RUNNING: {
		double curTime = GetCurTime();
		double deltaTime = curTime - lastLogTime;
		return wholeTime + deltaTime;
		break;
	}
	case CppUtility::Other::Timer::ENUM_STATE_STOP:
		return wholeTime;
		break;
	default:
		return -1.0;
